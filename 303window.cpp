...
void calculateFreqSweep(bool accent) {
    ...
    for (int samp = 0; samp < length; ++samp) {
        ...
        if (samp % 35 == 0) {
            plot_vcf_x[reduced_x] = samp;
            float freq = vcf_env_freq(env, fVco, fVmod_amt, 0.0, 0.0);
...
void show303Window() {
    ImGui::Begin("303 workbench", NULL, ImGuiWindowFlags_NoResize|ImGuiWindowFlags_NoCollapse);
    ...
    if (ImGui::InputFloat("D - 5.0 - exp accent scaling", &D, 0.01, 0.1)) {
        do_update |= true;
    }
